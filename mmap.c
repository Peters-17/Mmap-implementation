...
static double *sqrts;
static double *oldPage;
static double *newPage;
...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
  ...
  munmapResult = munmap(oldPage, page_size);
  if (errorFlag == 1 && munmapResult == -1) {
    printf("Munmap Error; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  //rounds a fault_addr down to page_size
  newPage = (double*) align_down(fault_addr,page_size);
  newPage = mmap(newPage, page_size , PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (newPage < 0) {
    printf("Mmap Error; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  //Use the function calculate_sqrts() to compute the correct square root values
  calculate_sqrts(newPage,(int)(newPage-sqrts),(int)(page_size/8));
  //update current page
  oldPage = newPage;
  return;
}
